...
int main(void) 
{
    ...
    //int funcMenuPrincipalAluno();
      ...
      retornoatualizacao == CADASTRO_REALIZADO)
			{
...
{ 
  if(n >= qtdealuno) //CONDIÇÃO QUE VERIFICA SE A TENTATIVA DE INCLUSÃO EXCEDE O TAMANHO DO VETOR ALUNOS.
  {
	...
	if(opc == 1) //CONDIÇÃO QUE PERMITE A INFORMAÇÃO DA MATRÍCULA SOMENTE QUANDO O USUÁRIO OPTA POR INCLUIR ALUNO NO MENU. CASO OPTE POR ATUALIZAÇÃO DE ALUNO, A MATRICULA PERMANECERÁ A MESMA.
  {
